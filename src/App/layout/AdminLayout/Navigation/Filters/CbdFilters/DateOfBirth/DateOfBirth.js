...
class costumDoB extends Component {
  ...
  onData1Change = (data) => {
    ...
    this.props.changeTimePeriodCrtd(
      data
        ? {
            createdDate: moment(data).format("MM/DD/YYYY").toString(),
            dayes: dataDiff,
          }
        : null
    );
  ...
  onData2Change = (data) => {
    ...
    this.props.changeTimePeriodExpr(
      data
        ? {
            expirationDate: moment(data).format("MM/DD/YYYY").toString(),
            dayes: dataDiff,
          }
        : null
    );
  ...
  render() {
    ...
    return <div className="auth-content">adfadfadf{timerPeriodContent}</div>;
  }
}

export default costumDoB;
